...
def calculate_leaves_details(filters, leave_type, employee):
    ...
    ledger_entries1 = get_leave_ledger_entries1(
        filters.from_date, filters.to_date, employee.name, leave_type)
    ...
    leaves_taken1 = remove_expired_leave(ledger_entries1)
    opening = get_leave_balance_on(
        employee.name, leave_type, add_days(filters.from_date, -1))

    new_allocation, expired_allocation = get_allocated_and_expired_leaves(
        ledger_entries, filters.from_date, filters.to_date)
    new_allocation1 = get_allocated_and_expired_leaves1(
        ledger_entries1, filters.from_date, filters.to_date)
    ...
    closing = max(opening + new_allocation -
...
def get_allocated_and_expired_leaves1(records1, from_date, to_date):
    ...
    for record in records1:
        if record.to_date < getdate(to_date) and record.total_leave_days > 0:
            new_allocation1 += record.total_leave_days
    return new_allocation1
...
def get_leave_ledger_entries1(from_date, to_date, employee, leave_type):
    records1 = frappe.db.sql("""
		SELECT
...
def get_department_leave_approver_map(department=None):
    conditions = ''
    if department:
        conditions = "and (department_name = '%(department)s' or parent_department = '%(department)s')" % {
            'department': department}

    # get current department and all its child
    department_list = frappe.db.sql_list(
        """ SELECT name FROM `tabDepartment` WHERE disabled=0 {0}""".format(conditions))  # nosec
