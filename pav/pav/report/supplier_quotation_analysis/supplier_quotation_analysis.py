...
def prepare_data(supplier_quotation_data, filters):
    ...
    group_by_field = "supplier_name" if filters.get(
        "group_by") == "Group by Supplier" else "item_code"
    company_currency = frappe.db.get_default("currency")
    float_precision = cint(frappe.db.get_default("float_precision")) or 2

    for data in supplier_quotation_data:
        # get item or supplier value for this row
        ...
        if supplier_currency:
            exchange_rate = get_exchange_rate(
                supplier_currency, company_currency)
        else:
            exchange_rate = 1

        row = {
            # leave blank if group by field
            "item_code":  "" if group_by_field == "item_code" else data.get("item_code"),
        ...
        row["price_per_unit"] = flt(
            row["price"]) / (flt(data.get("stock_qty")) or 1)
    ...
    for group in groups:
        # all entries pertaining to item/supplier
        ...
        if highlight_min_price:
            prices = [group_entry["price_per_unit"]
    ...
    if filters.get("item_code"):
        ...
        chart_data = prepare_chart_data(
            suppliers, qty_list, supplier_qty_price_map)
...
def get_columns(filters):
    ...
    # add positioned group by columns to the report
    columns[0:0] = group_by_columns
    return columns
